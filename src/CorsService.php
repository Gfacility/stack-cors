...
class CorsService
{
    ...
    private function normalizeOptions(array $options = array()): array
    {
        ...
        if ((config('app.server') == 'prod') && 
                (
                    str_contains($requestURL, 'api/v1') || 
                    str_contains($requestURL, 'api/visitor-kiosk/v1') ||
                    str_contains($requestURL, 'api/room-signage/v1')
                )
            ) {
            ...
            if (config('app.server') == 'prod') {
                ...
                $options['allowedOrigins'] = config('cors.device_environment_origins')['uat'];
            } else {
